...
var (
	windRegexp    = regexp.MustCompile(`\d* METAR.*EGLL \d*Z [A-Z ]*(\d{5}KT|VRB\d{2}KT).*=`)
...
func extractWindDirection(metars []string) []string {
	...
	for _, metar := range metars {
		if windRegexp.MatchString(metar) {
			winds = append(winds, windRegexp.FindAllStringSubmatch(metar, -1)[0][1])
		}
	}
	return winds
}
...
func main() {
	absPath, _ := filepath.Abs("/Users/sadegh/Code/Golang/wind-direction/metarfiles")
	...
	for _, file := range files {
		...
		metarsRepost := parseToArray(text)

		windDirections := extractWindDirection(metarsRepost)
